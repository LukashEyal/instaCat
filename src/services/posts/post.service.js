...
async function addComment(payload) {
  const post = await httpService.post(`post/add/comment/${payload.postId}`, payload)
  return post
}

async function deleteComment(payload) {
  const post = await httpService.post(`post/delete/comment/${payload.postId}`, payload)
  return post
}

async function activateMassLikes(postId) {
  const post = await httpService.get(`post/mass_like/${postId}`)
  return post
}
